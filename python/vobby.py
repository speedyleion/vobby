"""
This is a twisted TODO IS IT A PLUGIN OR AN APPLICATION OR WHAT? which
implements the main Vim to infinoted communication.

It will launch a server for Vim to Netbeans communication, and will connect that
server to an infinoted server for the infintoed communication.
...
class VobbyService(service.Service):
    """
    This will handle communication between the Vim protocol and the Infinoted
    protocol.

    The idea is that each protocol will register itself with this service when
    created, via the :meth:`add_protocol` method.  Then these protocols can then
    call back into this service with generic editing operations that each
    instance will know how to handle.
    ...
    def add_protocol(self, protocol):
        """
        This will add the given `protocol` instance to this object.  If a
        protocol instance that matches the `protocol` type already exists it
        will be replaced.
    ...
    def startService(self):
        ...
        # TODO this may need to be somewhere else.  Anyway kick off the ifinoted
        # client.  and now it's super hacky
